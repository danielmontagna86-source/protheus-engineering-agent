...
|---|---|---|
| G0 baseline integrity | lint/type/build/tests, configured mutation >=95%, `git diff --check` | Public `main` through `c354e98` passed 365 tests plus the protected Windows/Linux Node 22/24, Extension Host, mutation/dependency, secret, OSV and CodeQL matrix. The configured mutation score remains 95,05%. A future Stable artifact still requires its own exact rebuild. |
...
| G5 official Postgres | loopback/read-only/named-query/teardown evidence | PARTIAL — em 2026-09-25, o PostgreSQL 16 descartável continha 148 tabelas e o dicionário físico `990` (`SX2990`/`SX3990`/`SIX990`) estava `READY`, sem duplicidades, órfãos ou índices inválidos. O adapter provider-neutral passou named queries, binds, negações, limites, redação, timeout, cancelamento, concorrência e MCP. Uma role efêmera somente leitura passou a matriz, teve escrita negada e foi removida sem resíduo. O produto não embute driver/credencial. Resta repetir no artefato Stable exato e publicar uma receita de host suportada. |
| G6 authorized AppServer | compile success/error + RPO/TDS/CP1252 evidence | PARTIAL — a evidência consolidada está em `docs/qa/docker-appserver-pea-homologation-2026-09-24.md`. O candidato `0.3.9` vinculado a `ef09aac` passou no TDS 2.1.4/AppServer 24.3.1.5: conexão direta positiva/C2090, ponte pública fail-closed, cancelamento, timeout, AppServer indisponível e RPO custom indisponível com rollback. Todos os serviços e WebApp HTTP 200 foram restaurados. O responsável atestou origem Portal TOTVS do RPO; `TOP_NO_LICENSE` é estado comercial, não falha funcional. Restam contenção concorrente de lock do RPO e repetição no artefato Stable exato. A prova não implica certificação ou suporte oficial TOTVS. |
