...
| G6 authorized AppServer | compile success/error + RPO/TDS/CP1252 evidence | PARTIAL — o histórico manual e os primeiros recibos estão em `docs/qa/live-appserver-tds-homologation-2026-09-13.md`. Em 2026-09-24, o candidato exato `0.3.9`/`d033443` e seu VSIX SHA-256 `d61e7b9...` passaram novamente no TDS 2.1.4/AppServer 24.3.1.5: caso CP1252/LF válido `completed`/0 erros e caso inválido `failed`/C2090, tanto na conexão TDS direta como na ponte pública do PEA. O laboratório também passou recuperação real de AppServer/DBAccess, WebApp HTTP 200 e catálogo PostgreSQL somente leitura. O responsável atestou que o RPO foi obtido no portal oficial TOTVS; `TOP_NO_LICENSE` é registrado como estado do laboratório, não como falha funcional. A execução e seus limites estão em `docs/qa/docker-appserver-pea-homologation-2026-09-24.md`. Restam os casos live de timeout/cancelamento/RPO indisponível e a repetição no futuro artefato Stable. A prova não implica certificação ou suporte oficial TOTVS. |
