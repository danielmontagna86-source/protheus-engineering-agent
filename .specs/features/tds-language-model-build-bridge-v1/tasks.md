...
| T5 | Rodar regressão, host VS Code e revisão de código; empacotar VSIX. | T4 | `npm run validate:release-candidate` passa | Em execução |
| T6 | Exercitar no AppServer de laboratório pelo VSIX instalado. | T5 | relatório saneado com alvo, resultado e limites | Pendente — requer Extension Host do perfil autenticado |
