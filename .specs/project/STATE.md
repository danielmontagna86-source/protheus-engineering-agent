...
**Current Work:** O último candidato público integralmente validado é `d033443f1c5dede967fa9c7eac6a904a4f7bc016`, após os PRs #42 e #43. Esse commit passou a matriz pública e uma repetição local completa em checkout limpo: 357/357 testes, mutação 95,05%, audit sem vulnerabilidades, hosts VS Code atual/mínimo/TDS, lifecycle 0.3.8→0.3.9 e artefatos reproduzíveis. Em 2026-09-24, o VSIX exato SHA-256 `d61e7b9...` passou novamente na ponte PEA → TDS 2.1.4 → AppServer 24.3.1.5 para o par positivo/negativo, após correção e testes reais de recuperação do laboratório Docker. O responsável atestou que o RPO foi obtido no portal oficial TOTVS; `TOP_NO_LICENSE` permanece documentado, mas não é falha funcional para este laboratório privado nem requisito do produto distribuído. A prévia pública `v0.3.9` continua vinculada ao commit `29993a8`; a documentação posterior ainda não é uma nova release nem Stable. Stable/Marketplace permanecem `NO-GO` até os casos live restantes, UAT/piloto, publisher e evidência final existirem para um único artefato.
...
**Update 2026-09-24:** O laboratório Docker autorizado foi corrigido e homologado com o candidato exato `d033443`: reinício automático real, WebApp HTTP 200, TDS 2.1.4 direto e ponte PEA 0.3.9 positivos/negativos passaram. O registro é `docs/qa/docker-appserver-pea-homologation-2026-09-24.md`. O responsável atestou que o RPO veio do portal oficial TOTVS. `TOP_NO_LICENSE` é uma limitação comercial registrada, não uma falha funcional do laboratório; G6 permanece PARTIAL somente pelos casos live de timeout/cancelamento/RPO indisponível e pela futura repetição no artefato Stable. Nenhum bypass foi aplicado e nenhuma certificação oficial é alegada.
