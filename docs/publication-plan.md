...
**Current decision:** NO-GO for public release. The canonical repository exists privately and the standalone packaged VSIX installs and runs all four commands locally on isolated VS Code 1.95.3 and 1.133.0 hosts without Hermes. The revised candidate still needs green CI/OSV, clean-commit artifacts, final reviews, downloaded-asset reproduction and named approval.
...
7. Fresh-install the packaged VSIX in isolated Extension Hosts without Hermes and record evidence. Minimum 1.95.3 and current 1.133.0 passed locally; the revised candidate CI remains pending.
...
- Register and verify the final Visual Studio Marketplace publisher identifier before removing `private: true` from the extension manifest or attempting Marketplace publication.
