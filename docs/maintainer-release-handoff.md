...
2. **Real Protheus acceptance:** the authenticated disposable lab now has correlated direct TDS, public `pea.compileWithTds`, AppServer/RPO-unavailable and PostgreSQL/dictionary evidence for the 0.3.9 candidate. The positive bridge remains deliberately `unverified/TDS_COMPILE_SUCCESS_UNPROVEN` because the public TDS tool does not expose RPO-commit proof. See `docs/qa/docker-appserver-pea-homologation-2026-09-24.md`. Before Stable, repeat the matrix against the exact Stable artifact and exercise concurrent RPO lock contention; production deployment remains outside the product contract.
