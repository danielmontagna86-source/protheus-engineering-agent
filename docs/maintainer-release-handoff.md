...
2. **Real Protheus acceptance:** a disposable `Protheus Lab Local` AppServer/RPO compiled an identified CP1252/LF fixture with `TOTVS.CH` and rejected a controlled-invalid fixture with rollback on 2026-09-13; the initially recorded positive result was invalidated because it targeted the TDS output channel. See `docs/qa/live-appserver-tds-homologation-2026-09-13.md`. The candidate now adds the public `pea.compileWithTds` bridge; homologate it in the authenticated lab profile before retaining correlated evidence. Production/RPO deploy is outside the preview.
