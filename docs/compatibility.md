...
|---|---|---|
| VS Code | 1.95.3 minimum through current stable | Installed preview VSIX passed 1.95.3 and 1.139.0; later release artifacts require their own checks |
...
Remote SSH/WSL/container workspaces are experimental in Preview and are not in
the Stable support matrix. Package placement and the bundled Linux runtime have
passed in WSL, but the 2026-09-24 operator-facing activation probe was blocked
by the current Remote WSL host and produced no command receipt. See
`docs/qa/remote-extension-host-acceptance.md`. Docker is not required by the
installed product.
