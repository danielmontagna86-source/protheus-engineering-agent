...
| Release integrity code | PASS — source is reproduced from the declared Git commit; the VSIX embeds that commit and is byte-compared with an independent source rebuild; the SBOM reconciles every production package and edge from the exact lockfile; all hashes are manifest-bound |
