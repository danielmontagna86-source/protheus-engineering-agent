...
| Gate | Result |
|---|---|
| Automated suite | PASS — 294 tests, 0 failed, 0 skipped |
...
| Release integrity code | PASS — source is reproduced from the declared Git commit, VSIX is packaged twice byte-identically, SBOM reconciles the exact lockfile, and all hashes are manifest-bound |
...
- The source ZIP, VSIX and CycloneDX SBOM are the only release payloads, and all three must match the manifest.
